...
export function OrderReviewBody({ task }: { task: Task }) {
  ...
  return (
    <div>
      {task.needsClientInput && (
        <div className="mb-5 flex items-center gap-2 border border-red-300 px-4 py-3 text-sm text-red-600">
      ...
      {notice && (
        <p className="mb-4 text-sm text-gray-600 bg-gray-100 border border-gray-200 px-3 py-2">{notice}</p>
      )}
      ...
      {tab === "information" ? (
        <div className="border border-gray-200">
          ...
          <div className="px-5 py-4 border-t border-gray-100">
            <textarea
              ...
              rows={2}
              className="w-full border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand/40 focus:border-transparent mb-3"
            />
            <div className="flex gap-2">
              <button
                onClick={() => submitDecision("approve")}
                disabled={busy}
                className="bg-brand px-5 py-2 text-sm font-semibold text-white hover:bg-brand-hover disabled:opacity-50 transition-colors"
              >
                Approve
              </button>
              <button
                ...
                disabled={busy}
                className="border border-gray-300 px-5 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-50 transition-colors"
              >
                Submit Feedback
              </button>
            </div>
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mb-3">Visible to your account team. Separate from internal notes.</p>
          <div className="border border-gray-200 p-6 text-center text-sm text-gray-400 mb-4">No comments yet.</div>
          <div className="bg-gray-50 border border-gray-200 p-3">
            ...
            <div className="flex justify-end">
              <button
                ...
                className="bg-brand px-4 py-1.5 text-sm font-semibold text-white hover:bg-brand-hover disabled:opacity-50 transition-colors"
              >
                Submit
              </button>
            </div>
