...
export default function ScrollTestimonials() {
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      <div ref={stickyRef} className="sticky overflow-hidden" style={{ backgroundColor: "#f8fafc" }}>
        {/* Ambient */}
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: "radial-gradient(ellipse at 50% 50%, rgba(29,78,216,0.05) 0%, transparent 70%)" }} />

        {/* Header */}
        <div className="relative z-20 pt-7 pb-4 px-6 text-center flex-shrink-0 border-b border-slate-200">
          <div className="max-w-5xl mx-auto flex items-center justify-between">
            <div className="text-left">
              <p className="text-blue-600 text-xs font-semibold uppercase tracking-widest mb-1">Testimonials</p>
              <h2 className="text-2xl md:text-3xl font-extrabold text-slate-900">Don&apos;t take our word for it</h2>
            </div>
            {/* Progress dots */}
              ...
              {testimonials.map((_, i) => (
                <div key={i} className="rounded-full transition-all duration-700"
                  style={{
                    ...
                    height: "5px",
                    backgroundColor: i === activeIndex ? "#2563eb"
                      : i < activeIndex ? "rgba(37,99,235,0.3)"
                      : "rgba(0,0,0,0.1)",
                  }} />
              ))}
            </div>
                ...
                >
                  <div
                    style={{
                      ...
                      borderRadius: "16px",
                      border: isActive ? "1px solid rgba(37,99,235,0.3)" : "1px solid rgba(0,0,0,0.07)",
                      boxShadow: isActive ? "0 20px 60px rgba(37,99,235,0.12), 0 0 0 1px rgba(37,99,235,0.18)" : "none",
                      padding: "20px",
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: "3px", marginBottom: "10px" }}>
                      {[...Array(5)].map((_, si) => (
                        <svg key={si} width="12" height="12" fill="#2563eb" viewBox="0 0 20 20">
                    ...
                    <p style={{ color: "#475569", fontSize: "0.8rem", lineHeight: "1.55", flex: 1, overflow: "hidden" }}>
                      &ldquo;{t.quote}&rdquo;
                    </p>

                    {/* Author */}
                    <div style={{ marginTop: "12px" }}>
                      <div style={{ color: "#0f172a", fontSize: "0.8rem", fontWeight: 600 }}>{t.name}</div>
                      <div style={{ color: "#94a3b8", fontSize: "0.7rem", marginTop: "2px" }}>{t.role}</div>
                    </div>
                  </div>
                </div>
              );
        ...
        {/* Scroll hint */}
        {activeIndex < 0 && (
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-slate-400 text-xs z-20 animate-pulse">
